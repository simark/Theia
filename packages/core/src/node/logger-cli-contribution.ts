...
export class LogLevelCliContribution implements CliContribution {
    ...
    protected logLevelsChangeEvent: Emitter<void> = new Emitter<void>();
    ...
    async setArguments(args: yargs.Arguments): Promise<void> {
        ...
        if (args['log-levels-file'] !== undefined) {
            let filename = args['log-levels-file'];
            filename = path.resolve(filename);

            await this.slurpLogLevelsFile(filename);
            await this.watchLogLevelsFile(filename);
        }
    }

    protected watchLogLevelsFile(filename: string): Promise<void> {
                ...
                for (const event of events) {
                    switch (event.action) {
                        ...
                        case nsfw.actions.MODIFIED:
                            await this.slurpLogLevelsFile(filename);
                            this.logLevelsChangeEvent.fire(undefined);
                            break;
                    }
                }
            } catch (e) {
                console.error(`Error reading log levels file ${filename}: ${e}`);
            }
    ...
    protected async slurpLogLevelsFile(filename: string) {
        try {
            ...
            if ('default' in data) {
                newDefaultLogLevel = this.readLogLevelString(data['default'], `Unknown default log level in ${filename}`);
            }
    ...
    get onLogLevelsChange(): Event<void> {
        return this.logLevelsChangeEvent.event;
    }
